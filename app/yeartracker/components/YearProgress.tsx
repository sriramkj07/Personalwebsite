...
export default function YearProgress() {
  ...
  const calculateCellSize = () => {
    ...
    const availableWidth = Math.max(280, containerWidth - 32) // minimum 280px, padding 16px each side
  ...
  return (
    <div className="min-h-screen bg-background p-4 sm:p-8 flex justify-center items-center">
      <Card className="bg-background text-foreground w-full max-w-full overflow-x-auto">
        <CardContent className="p-4" id="year-progress-container">
          <div className="mb-6">
            <div className="text-lg mb-1">Year Progress – {state.progress.toFixed(2)}% Complete</div>
            <div className="text-sm text-muted-foreground">
              {Math.floor(state.daysElapsed / 7)} weeks elapsed • {52 - Math.floor(state.daysElapsed / 7)} weeks
              remaining
            </div>
          ...
          <div className="min-w-[280px]" style={{ width: gridWidth }}>
            <div style={{ position: "relative", height: 20, marginBottom: 4 }}>{monthLabels()}</div>
